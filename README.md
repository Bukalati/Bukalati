<div align="center">

# ALIREZA NOORMOHAMMAD

### UI/UX DESIGNER × FRONT-END DEVELOPER

**I design interfaces and turn them into real digital products.**

<br />

[Portfolio](#) · [LinkedIn](https://www.linkedin.com/in/alireza-noormohammad-037473289/) · [Email](mailto:alireza.nourasi@gmail.com)

</div>

---

## 01 / ABOUT

I'm a **Computer Engineering student** with a background in **UI/UX Design**, currently focusing on Front-end Development.

I enjoy working across the gap between **design and code** — turning ideas and interfaces into products that are clear, responsive, functional, and intentional.

Currently, I'm going deeper into **React, TypeScript, and Next.js** while building real-world projects and improving my front-end engineering workflow.

---

## 02 / WHAT I DO

<table>
<tr>
<td width="33%" valign="top">

### DESIGN

UI/UX Design
Product Design
Design Systems
Prototyping
Responsive Design
Figma

</td>

<td width="33%" valign="top">

### BUILD

React
Next.js
TypeScript
JavaScript
Tailwind CSS
HTML / CSS

</td>

<td width="33%" valign="top">

### EXPLORE

Creative Development
Interaction Design
Digital Products
Visual Storytelling
Front-end Architecture

</td>
</tr>
</table>

---

## 03 / TECH STACK

### Front-end

<p>
<img src="https://skillicons.dev/icons?i=html,css,js,ts,react,nextjs,tailwind" />
</p>

### Design

<p>
<img src="https://skillicons.dev/icons?i=figma" />
</p>

### Programming & Tools

<p>
<img src="https://skillicons.dev/icons?i=cpp,python,cs,git,github,linux,vscode" />
</p>

---

## 04 / SELECTED WORK

### `01` — Personal Portfolio

A scroll-focused personal portfolio designed and developed from scratch with Next.js.

**Stack:** `Next.js` `TypeScript` `Tailwind CSS`

→ [View Repository](#)

---

### `02` — Socially

A responsive admin dashboard built with React and TypeScript, focused on reusable components, routing, forms, and API-driven interfaces.

**Stack:** `React` `TypeScript` `Tailwind CSS` `React Router`

→ [View Repository](#https://github.com/Five-o-One/Socially)

---

### `03` — ThinkBoard

A fintech product focused on credit scoring and loan discovery, with an emphasis on product experience and conversion-oriented flows.

**Focus:** `UI/UX` `Product Design` `PWA` `Fintech`

→ [View Repository](#https://github.com/Bukalati/Thinkboard-FrontEnd)

---

## 05 / CURRENTLY

```text
→ Building my personal portfolio with Next.js
→ Going deeper into TypeScript
→ Improving React architecture & component design
→ Exploring creative front-end development
→ Turning UI/UX concepts into real products
```

---

## 06 / GITHUB ACTIVITY

<div align="center">

<img src="https://github-readme-stats.vercel.app/api?username=Bukalati&show_icons=true&hide_border=true&rank_icon=github" height="170" />

<img src="https://github-readme-streak-stats.herokuapp.com/?user=Bukalati&hide_border=true" height="170" />

</div>

---

## 07 / CONTRIBUTIONS

<div align="center">

<img src="https://github-readme-activity-graph.vercel.app/graph?username=Bukalati&hide_border=true&area=true" width="100%" />

</div>

---

## 08 / A LITTLE MORE

Outside of code and design, I'm interested in:

**Digital Art · Character Design · Creative Development · Technology · Visual Storytelling**

---

<div align="center">

### DESIGN → BUILD → ITERATE

*Good interfaces don't happen by accident.*

<br />

**Let's build something meaningful.**

<br />

[Portfolio](#) · [LinkedIn](https://www.linkedin.com/in/alireza-noormohammad-037473289/) · [Email](mailto:alireza.nourasi@gmail.com)

</div>
